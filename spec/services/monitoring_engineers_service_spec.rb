    ...
    it 'does add the engineers based in new implementation without the min_shift' do
      ...
      (0..6).each do |wday|
        ...
        schedules.find_by(week_number: week_day.wday).hours_range.each_with_index do |hour, index|
            ...
            case wday
            when 0 then [engineer1.id, engineer2.id].each { |id| create(:service_engineer, service_id: service.id, engineer_id: id) }
            when 1 then [engineer2.id, engineer3.id].each { |id| create(:service_engineer, service_id: service.id, engineer_id: id) }
            end
        end
      end
      ...
      expect(services.map { |service| service.engineer.first_name }.uniq).to eq(["Benjamin", "Ernesto"])
      
      # Thuesday, full for Benjamin
      services = company.services.by_range((beginning_of_week + 1).beginning_of_day, (beginning_of_week + 1).end_of_day)
      
      expect(services.map { |service| service.engineer.first_name }.uniq).to eq(["Ernesto", "Barbara"])
