class ServicesController < ApplicationController
    ...
    def build_service_schedule
        render_to_string(partial: "services/index/service_schedule", 
                         locals: { company:          company, week: week, weeks: weeks, company_schedules: company_schedules, 
                                   company_services: company_services }, 
                         layout: false, formats: :html)
    end
    
    def companies
        @companies ||= Company.includes(:schedules, [services: [:service_engineers, :engineers, :engineer]]).all 
    end
